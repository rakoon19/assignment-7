...
const FriendsList = () => {
  ...
  const getTagStyle = (tag) => {
    ...
    if (tagLower.includes("hobby"))
      return "bg-[#D1FAE5] text-[#065F46] border-[#A7F3D0]";
    if (tagLower.includes("travel"))
      return "bg-[#D1FAE5] text-[#065F46] border-[#A7F3D0]";
    if (
      tagLower.includes("college") ||
      tagLower.includes("high school")
    )
      return "bg-[#DBEAFE] text-[#1E40AF] border-[#BFDBFE]";
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {friend.tags.slice(0, 2).map((tag, index) => (
                  <span
                    key={index}
                    className={`px-3 py-1 rounded-full text-xs font-medium uppercase border ${getTagStyle(
                      tag
                    )}`}
                  >
                    {tag}
